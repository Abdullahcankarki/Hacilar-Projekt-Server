 ...
 * Ein Fahrzeug laden.
 */
export async function getFahrzeugById(
  id: string
): Promise<FahrzeugResource | null> {
...
export async function listFahrzeuge(params?: {
  ...
  const totalDocsAll = await FahrzeugModel.estimatedDocumentCount();
  const limit =
...
export async function updateFahrzeug(
  ...
  if (patch.kennzeichen !== undefined)
    update.kennzeichen = normalizeKennzeichen(patch.kennzeichen);
  if (patch.name !== undefined) update.name = normalizeText(patch.name) ?? null;
  if (patch.maxGewichtKg !== undefined)
    update.maxGewichtKg = Number(patch.maxGewichtKg);
  if (patch.aktiv !== undefined) update.aktiv = !!patch.aktiv;
  if (patch.regionen !== undefined)
    update.regionen = normalizeRegionen(patch.regionen) ?? [];
  if (patch.samsaraVehicleId !== undefined)
    update.samsaraVehicleId = normalizeText(patch.samsaraVehicleId) ?? null;
  if (patch.bemerkung !== undefined)
    update.bemerkung = normalizeText(patch.bemerkung) ?? null;
...
export async function deleteFahrzeug(id: string): Promise<void> {
  ...
  try {
    await session.withTransaction(async () => {
      const deleted = await FahrzeugModel.findByIdAndDelete(id).session(
        session
      );
...
export async function deleteAllFahrzeuge(): Promise<void> {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const fahrzeuge = await FahrzeugModel.find({}, { _id: 1 }).session(
        session
      );
